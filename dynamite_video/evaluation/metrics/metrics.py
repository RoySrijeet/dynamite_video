...
def compute_j_and_f(
    ...
    `target_ids` must not contain VOID class, if applicable for the dataset.

    Args:
        y_true: ground truth labels, np.ndarray, shape T,H,W
        y_pred: predicted labels, np.ndarray, shape T,H,W
        target_ids: list of IDs of targets present in `y_true` 
    ...
    if len(target_ids) == 0:
        raise ValueError('Number of targets in y_true should be higher than 0.')

    jaccard = batched_jaccard(y_true, y_pred, target_ids, ignore_label)
    f_measure = batched_f_measure(y_true, y_pred, target_ids, ignore_label)
