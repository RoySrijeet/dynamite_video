...
class AvgClicksPoolingInitializer(nn.Module):
    ...
    #     H = float(h*8)
    #     W = float(w*8)
    #     no_click_query = nn.Parameter(torch.zeros(1,1,d)).to(device)    # TODO - move to __init__
        
    #     descriptors = []
        
    #     for fr_idx, fr_fg_coords in enumerate(batched_fg_coords_list):
            
    #         # merge fg and bg clicks on any given frame
    #         clicks = []
            
    #         # flatten nested list of fg clicks
    #         clicks.extend(chain.from_iterable(fr_fg_coords))
    #         # add bg clicks
    #         clicks.extend(batched_bg_coords_list[fr_idx])
            
    #         if len(clicks) == 0:
    #             descriptors.append(no_click_query)
    #             continue
            
    #         clicks_backup = copy.deepcopy(clicks)
    ...
    #         click_coords[:,1]/=W
    #         # invert (y,x) -> (x,y)
    #         click_coords = click_coords.flip(-1)
    ...
    #             nbd_features = self.get_features_descriptors(fmap_scale_fr, click_coords.unsqueeze(0))
                
    #             query_descriptors.append(nbd_features)
    #             #TODO
    #             # instance_feature = ...
    #             # temporal_comp = ...
