...
def sigmoid_ce_loss(
    ...
    # loss = F.binary_cross_entropy_with_logits(inputs.flatten(0,1), targets.flatten(0,1), reduction="none")
...
class SetFinalCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, num_masks, num_queries_per_object = None):
        ...
        # target_masks = torch.stack(masks).to(dtype=torch.float32)
    ...
    def forward(self, outputs, targets, instance_ids, num_queries_per_object = None):
        ...
        for i,t in enumerate(targets):
            ...
            targets[i]["instance_masks"] = torch.cat((t["instance_masks"], target_bg_mask.unsqueeze(0)), dim=0)   
        
        # Compute the average number of target boxes accross all nodes, for normalization purposes
        # num of masks to be predicted = (num of instances in each frame + BG) * num of frames
        num_masks = (len(instance_ids) + 1) * len(targets)
